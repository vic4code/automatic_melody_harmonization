...
class MusicCVAE(nn.Module):
    def __init__(self,
        ...
        # Encoder to latent
        self.hidden2mean = nn.Linear(encoder_hidden_size * encoder_num_layer * 2, latent_size)
        self.hidden2logv = nn.Linear(encoder_hidden_size * encoder_num_layer * 2, latent_size)
        ...
        # Latent to decoder
        self.latent2conductor_input = nn.Linear(latent_size, Constants.NUM_CHORDS)  
        ...
        self.conductor = nn.GRU(input_size=Constants.NUM_CHORDS + prenet_hidden_size * prenet_num_layer * 2, 
                                ...
                                num_layers=conductor_num_layer, 
                                batch_first=True, 
                                bidirectional=False)
        
        self.conductor_embedding = nn.Sequential(
            nn.Linear(in_features=conductor_hidden_size, out_features=latent_size),
            nn.Tanh()
        )
        
        self.decoder = nn.GRU(input_size=latent_size + Constants.NUM_CHORDS + 2 * 12 * Constants.BEAT_RESOLUTION,
        ...
        self.outputs2chord = nn.Linear(decoder_hidden_size, Constants.NUM_CHORDS)
