...
import os
import random
from model.Objective_CVAE_Linear import *
from decode import *
...
def main():
    ...
    parser.add_argument(
        '--sample_num', default=5, help='sample number')
    parser.add_argument(
        '--cc', default=1, help='set pitch repeated pattern')
    ...
    for index in indices:
        ...
        batch_size = 1
        CC = torch.Tensor([float(args.cc)])
#         r_rhythm = torch.Tensor([float(args.rhythm_ratio)])

        CC = CC.view(batch_size,1,1).expand(batch_size,272,1).to(device)
...
#         z = torch.cat((latent,melody1,r_pitch,r_rhythm), dim=-1)
        z = torch.cat((latent,melody1,CC), dim=-1)

        _, chord_pred = model.decode(z)
...
#         filename = str(index) + '-pitch-' + str(args.pitch_ratio) + '-rhythm-' + str(args.rhythm_ratio)
        filename = str(index) + '-cc-' + str(args.cc)
        print(result_dir)
