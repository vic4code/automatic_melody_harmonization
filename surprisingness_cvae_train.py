...
from dataloader import SurprisingnessDataset
...
#         length = np.random.randint(1,272, size = (self.batch_size,))
        ...
        surprisingness = np.load('./data/surprisingness_all.npy')
        ...
        train_surprisingness = surprisingness[self.val_size:]
        val_surprisingness = torch.from_numpy(surprisingness[:self.val_size]).float()
        
        weight_chord = torch.from_numpy(weight_chord).float().to(self.device)
        
        # Create dataloader
        print('creating dataloader...')
        dataset = SurprisingnessDataset(train_melody, train_chord_idx, train_length, train_chord_onehot, train_surprisingness)
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True, num_workers=16, drop_last=True)

        return dataloader, val_melody, val_chord_onehot, val_length, val_surprisingness
    ...
    def train(self, model,optimizer,dataloader,step,k,x0,loss_function):  
        ########## Training mode ###########
            ...
            for melody, _, length, chord_onehot, surprisingness in dataloader:
                ...
                melody, length, chord_onehot, surprisingness = melody.to(self.device), length.to(self.device).squeeze(), chord_onehot.to(self.device), surprisingness.to(self.device)
                optimizer.zero_grad()

                # Model prediction
        #         print(chord_onehot.shape)
        #         print(length.shape)
                pred, logp ,mu, log_var, _ = model(chord_onehot,melody,surprisingness,length)
    ...
    def eval(self,model,val_melody,val_chord_onehot,val_length,val_surprisingness,step,k,x0,loss_function):
        ########## Evaluation mode ###########
            model.eval()
            validation_loss = self.validation_loss
            melody, length, chord_onehot, surprisingness = val_melody.to(self.device), val_length.to(self.device).squeeze(), val_chord_onehot.to(self.device), val_surprisingness.to(self.device)
...
#                 torch.save(model.state_dict(), model_dir + '.pth')
    ...
    def run(self):
        ...
        epochs = self.epoch
        # Load data
        dataloader, val_melody, val_chord_onehot, val_length, val_surprisingness = self.load_data()
